...
import { ADD_PROFILE_IMG, DELETE_PROFILE_IMG, UPDATE_PROFILE } from "@/utils/constants";
...
function Profile() {
  ...
  const saveChanges = async () => {
    if (validateProfile()) {
      try {
        const res = await apiClient.post(UPDATE_PROFILE, { firstName, lastName, color: selectedColor, image }, { withCredentials: true });
  ...
  const handleImageDelete = async () => {
    try {
      const res = await apiClient.delete(DELETE_PROFILE_IMG, { withCredentials: true });
      if (res.status === 200 && res.data.data.image === null) {
      ...
      <div className="flex flex-col gap-10 w-[80vw] md:w-max">
        ...
        <div className="grid grid-cols-2">
          <div className="h-full w-32 md:w-48 md:h-48 relative flex items-center justify-center"
            ...
            {hovered && (
              <div className="absolute h-32 w-32 md:h-48 md:w-48 flex items-center justify-center bg-black/50 ring-fuchsia-50 rounded-full"
                onClick={image ? handleImageDelete : handleFileInputRef}>
                {image ? (
            ...
            <div className="w-full flex gap-5">
              {colors.map((color, index) => (
                <div className={`${color} h-8 w-8 rounded-full cursor-pointer transition-all duration-300
                  ${selectedColor === index ? "outline outline-white/40 outline-3" : ''}`}
                  key={index}
